...
type builtinUser struct {
    perfect.DBO         `bson:",inline,omitempty"`
...
func setupAdminAccount(module *perfect.Module) {
    ...
    //create a new user if it doesn't exist, then create a Perfect profile 
    if admin_user == nil {
        //create builtin user
        ...
        //link the builtin user and Perfect profile
...
func createBuiltinProfile(username, password, name, email string, db perfect.Database) (user *builtinUser, profile *perfect.User, err error) {
...
func newBuiltinUser(username string, password []byte, password_salt string) *builtinUser {
	return &builtinUser{
        DBO: perfect.DBO { Collection: BUILTIN_USER_COLLECTION },
		Id:   username,
		Password: password,
        Salt: password_salt,
	}
...
func findBuiltinUser(username string, db perfect.Database) (user *builtinUser, err error) {
