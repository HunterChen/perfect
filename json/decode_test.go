...
type W struct {
	I  int    `json:"i"`
	RS string `json:"rs,readonly"`
	RO T      `json:"ro,readonly"`
	RA []int  `json:"ra,readonly"`
	S  string `json:"s"`
}
...
type Top struct {
	Level0   int
	Level0ro int `json:"l0,omitempty,readonly"`
	Embed0
	*Embed0a
...
var unmarshalTests = []unmarshalTest{
	...
	//readonly fields
	{in: `{"i":42, "ro":{"x":"abc","y":2,"z":3}, "rs":"abc", "ra":[1,2,3], "s":"abc"}`, ptr: new(W), out: W{I: 42, S: "abc"}},
...
func TestReadOnlyTag(t *testing.T) {
	input := []byte(`{"i":42,"ro":{"x":"abc","y":2,"z":3},"rs":"abc","ra":[1,2,3],"s":"abc"}`)
	actual := &W{I: 99, RO: T{X: "xyz", Y: 42, Z: 42}, RS: "xyz", RA: []int{42, 42, 42}, S: "xyz"}
	expected := &W{I: 42, RO: T{X: "xyz", Y: 42, Z: 42}, RS: "xyz", RA: []int{42, 42, 42}, S: "abc"}
	...
	expected_json := []byte(`{"i":42,"rs":"xyz","ro":{"X":"xyz","Y":42},"ra":[42,42,42],"s":"abc"}`)

	actual_json, err := Marshal(actual)
	if err != nil {
		t.Fatalf("err = %v", err)
	}
